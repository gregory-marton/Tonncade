...
*/
/**
 * board.js - State management for the two bounded boards: Blast's radius-5 hexagon and Gravity's
 * cup. These used to be ONE shared `Board` object whose shape-dependent methods branched on
 * `App.currentMode === 'gravity'` -- meaning Blast and Gravity, two mechanically distinct games,
 * silently read and wrote the same `cells` Map. Placing in one could leave debris the other later
 * collided with, and neither mode's board was independent of the other's (INV-48, #15/#16's
 * sibling bug for Blast/Gravity). createBoard(shape) now makes each mode its own instance, with
 * its shape fixed at construction instead of read from whatever mode happens to be active when a
 * method runs.
 *
 * Coordinates are axial (p, q).
 *
 * Blast axes (radius-5 hexagon, 11 cells across at widest point):
...
function createBoard(shape) {
    return {
        shape, // 'blast' | 'gravity' -- fixed for this instance, never read from App.currentMode
        radius: 5,
        cells: new Map(), // Key: "p,q", Value: { type, color }

        // Check if a cell is within the active game bounds
        isInBounds: function(p, q) {
            if (this.shape === 'gravity') {
        ...
        /**
         * Detection logic for completed lines: Gravity's horizontal cup rows, or Blast's three
         * axes (f, m, t) -- a straight sequence from one boundary to the other.
         */
        ...
        checkActivePlacement: function(type, p, q, rotation) {
            ...
            // locked while overhanging leaves its off-grid cells in this.cells permanently
...
const Board = createBoard('blast');
const GravityBoard = createBoard('gravity');

if (typeof module !== 'undefined') {
    module.exports = { Board, GravityBoard, createBoard };
}
