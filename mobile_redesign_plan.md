...
- **#38 Gravity: guarantee a single-hex piece can always squeeze through**: a piece should be able to keep any single hex on the board and have enough room to move/drop, so a lone hex can get underneath things that aren't completely blocked. Currently a piece can get stuck against neighbors/walls with no legal move even though a single-hex gap exists — this is a real mobile-blocking bug (players get stuck with no way to continue), not a new-capability idea, so it belongs here rather than in `next_steps.md`. Needs investigation into `js/gravity.js`'s collision/move-validity checks to figure out where the "any single hex always has a legal path down" guarantee is being violated. High priority: next up after the current Sandbox gesture redesign and test-strengthening audit finish.
